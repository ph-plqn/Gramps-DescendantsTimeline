...
résoudre HARD
        ...
        │
        └── HARD cohérent ou absent
                │
                ▼
        confronter les SOFT au HARD
                │
          ┌─────┴─────┐
          │           │
     ...
     résoudre SOFT
          ...
          (ou aucun domaine
           si aucun HARD)
```
Étape 1 — sélectionner les contraintes du target

Toutes les contraintes traitées doivent concerner le `TemporalTarget` demandé.

Elles sont séparées selon deux dimensions :
```text
strength
├── HARD
└── SOFT
...
operator
├── AFTER_OR_EQUAL   → borne minimum
└── BEFORE_OR_EQUAL  → borne maximum
```
Étape 2 — résoudre les contraintes HARD

Pour les bornes inférieures :
```text
hard_minimum = MAX des bornes HARD >=
```
Pour les bornes supérieures :
```text
hard_maximum = MIN des bornes HARD <=
```
Si plusieurs contraintes déterminent exactement la même borne gagnante, elles sont toutes conservées dans le même `ResolvedBound`, dans leur ordre d'arrivée.

Une borne absente reste `None`.
...
hard_minimum = >= 1817
    provenance : les deux contraintes à 1817

hard_maximum = <= 1825
```
Étape 3 — détecter une contradiction HARD/HARD

Si les deux bornes existent :
```text
hard_minimum.value <= hard_maximum.value
```
le domaine HARD est cohérent. L'égalité est autorisée et représente une date ponctuelle.
...
```
Les contraintes déterminantes des bornes contradictoires sont conservées dans `conflicting_constraints`.

Le Resolver ne choisit aucune des deux et ne tente aucun raffinement SOFT.

Les deux `ResolvedBound` HARD restent néanmoins dans `ConstraintResolution` afin de pouvoir expliquer la contradiction :
```text
>= 1840 HARD
<= 1830 HARD
...
→ HARD_HARD
```
Étape 4 — initialiser le domaine raffiné

S'il existe un domaine HARD cohérent, le domaine raffiné commence comme une copie logique de celui-ci :
```text
refined_minimum = hard_minimum
refined_maximum = hard_maximum
```
Mais l'absence de HARD n'interdit pas le raffinement.

Les contraintes SOFT peuvent créer seules une ou deux bornes :
```text
aucun HARD
SOFT >= 1854
SOFT <= 1862
...
Une estimation raisonnable et explicitement présentée comme telle est préférable à l'absence totale d'information.

Étape 5 — confronter les SOFT au domaine HARD

Lorsqu'un domaine HARD existe, chaque contrainte SOFT est d'abord confrontée à celui-ci.

Une SOFT compatible peut participer au raffinement.

Une SOFT incompatible avec le HARD :
```text
HARD   [1817 -------- 1825]
SOFT B [1813--1816]
```
est exclue du calcul du domaine raffiné et produit un conflit :
```text
HARD_SOFT
```
Le HARD reste souverain.

Mais cette SOFT incompatible ne condamne pas les autres SOFT :
```text
HARD    [1817 -------- 1825]
SOFT A  [1812 ---- 1822]     → compatible
SOFT B  [1813-1816]          → incompatible

→ refined = [1817,1822]
→ conflict = HARD_SOFT
```
Autrement dit, conflit et raffinement sont deux informations indépendantes.
Étape 6 — résoudre les SOFT compatibles

Parmi les SOFT encore admissibles :
```text
soft_minimum = MAX des >=
soft_maximum = MIN des <=
```
Elles peuvent resserrer le HARD :
```text
HARD       [1817 -------- 1825]
SOFT >=1820
SOFT <=1822
...
→ refined  [1820 -- 1822]
```
ou constituer seules le domaine :
```text
HARD       aucun
SOFT >=1854
SOFT <=1862
...
→ refined  [1854 -------- 1862]
```
Une SOFT moins restrictive qu'un HARD n'élargit jamais le domaine :
```text
HARD >=1817
SOFT >=1812

→ refined_minimum = >=1817 HARD
```
C'est pourquoi un `refined_*` peut être un `ResolvedBound` de force HARD ou SOFT.
Étape 7 — détecter un conflit SOFT/SOFT

Des contraintes SOFT peuvent être individuellement acceptables mais incompatibles entre elles :
```text
HARD    [1817 -------- 1825]
...
SOFT B             [1823--1825]
```
On ne choisit pas arbitrairement A ou B.
...
conflict = SOFT_SOFT
```
Les raffinements SOFT participant au conflit sont abandonnés et les contraintes responsables sont conservées dans `conflicting_constraints`.
...
refined_minimum = None
refined_maximum = None
conflict_type   = SOFT_SOFT
```
Le moteur préfère donc ne rien estimer plutôt que choisir arbitrairement entre deux heuristiques contradictoires.
