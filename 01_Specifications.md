...
Chaque famille Gramps est prise en compte dans le modèle de descendance.

Une famille comportant deux individus peut être représentée par un nœud de mariage conformément aux règles des événements et aux cas particuliers.
...
## R010 — Unicité de représentation des descendances

Une même descendance n'est représentée qu'une seule fois.

Lorsqu'une branche de descendance est déjà représentée, le greffon n'en produit pas une seconde représentation.

Une référence à la première occurrence est affichée.
...
Les familles successives d'une personne sont représentées dans l'ordre chronologique lorsque cet ordre peut être déterminé à partir des dates connues ou des contraintes temporelles.

Lorsque l'ordre chronologique ne peut pas être établi, le greffon conserve l'ordre des familles défini dans Gramps.

Le greffon n'invente jamais un ordre chronologique.
...
## G012 — Référence à une descendance déjà représentée

Lorsqu'une descendance est déjà représentée ailleurs dans la timeline, le greffon affiche un renvoi vers cette représentation au lieu de la reproduire.
...
## P007 — Réutilisation des branches

Chaque branche de descendance est calculée une seule fois.

Les représentations ultérieures utilisent la branche déjà construite.
